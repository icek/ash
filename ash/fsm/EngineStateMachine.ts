...
export class EngineStateMachine
{
    ...
    public changeState( name:string ):void
    {
        ...
        let toAdd:ISystemProvider<any>[] = [];
        let id:any;
        for( let provider of newState.providers )
        {
        ...
        {
            for( let provider of this.currentState.providers )
            {
                id = provider.identifier;
                let other:ISystemProvider<any> = toAdd[ id ];
